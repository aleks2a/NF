Given(/^I launch my device with JutIO page$/) do
  launch_driver
  home_screen.get_home
end

Given(/^I launch my browser with JutIO page$/) do
  launch_browser
  home_screen.get_home
end
...
Then(/^I press on Login button$/) do
...
Then(/^I expect to see error "(.*?)"$/) do |message|
  page_actions.wait_for(60){signin_screen.errors.text != ""}
